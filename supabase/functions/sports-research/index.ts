  ...
  properties: {
    ...
    matches: {
      ...
      items: {
        ...
        properties: {
          ...
          homeTeam: STRING,
          awayTeam: STRING,
          predictedOutcome: { type: "string", enum: ["home", "draw", "away", "abstain"] },
    ...
    sources: {
      ...
      items: {
        ...
        properties: { title: STRING, url: STRING, publisher: STRING, publishedAt: NULLABLE_STRING },
      },
    },
...
Deno.serve(async (req: Request) => {
  ...
  if (!apiKey)
    return json({ error: "Le secret OPENAI_API_KEY n'est pas configuré dans Supabase." }, 500);
  ...
  if (request.length < 10 || request.length > 1_500) {
    return json({ error: "La demande doit contenir entre 10 et 1 500 caractères." }, 400);
  }

  const model = Deno.env.get("OPENAI_RESEARCH_MODEL") || "gpt-5.6-sol";
  const system = `Tu es l'agent de recherche football d'OddsIQ. Transforme la demande en analyse
...
défini, explique-le. Réponds en français et rappelle qu'un pronostic n'est jamais une garantie.`;
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      input: [
        ...
        { role: "user", content: `Date UTC : ${new Date().toISOString()}\n\nDemande : ${request}` },
      ],
      text: { format: { type: "json_schema", name: "sports_research", strict: true, schema } },
    }),
  });
  ...
  if (!output) return json({ error: "OpenAI n'a pas retourné d'analyse exploitable." }, 502);
  try {
